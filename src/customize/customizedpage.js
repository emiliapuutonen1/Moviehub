...
const CustomizedPage = ({ userId }) => {
  ...
  const apiBaseUrl = 'https://moviehub-sovellus1.onrender.com';
...
const CustomizedPage = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchUserPreferences = async () => {
      try {
        ...
        if (link) {
          ...
          if (!isNaN(parsedUserId)) {
            const response = await axios.get(`${apiBaseUrl}/api/user-preferences/${parsedUserId}`);
            const preferences = response.data;
            setUserPreferences(preferences);
            setPreferencesFetched(true); 
          } else {
    ...
    const generateShareableLink = async () => {
      try {
        if (userId && preferencesFetched) {
          ...
          if (!isNaN(parsedUserId)) {
            const linkResponse = await axios.post(`${apiBaseUrl}/api/generate-link`, {
              userId: parsedUserId,
            });
            const generatedLink = linkResponse.data.link;
            setShareableLink(generatedLink);
    ...
    fetchUserPreferences();
    generateShareableLink();
  }, [userId, link, preferencesFetched, apiBaseUrl]);
    ...
    <div className="CustomizedPageh">
      ...
      <div className="CustomizedPage">
        ...
        {shareableLink && (
          <div>
            <p>Share your page with friends!:</p>
            <p className="shlink">{`${apiBaseUrl}/share/${shareableLink}`}</p>
          </div>
        )}
      </div>
    </div>
