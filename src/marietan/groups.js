...
export const Groups = ({ userId }) => {
  ...
  const handleChangeGroupname = async () => {
    ...
    if (newGroupname) {
      try {
        const response = await axios.put(`${apiBaseUrl}/groups/${group_id}`, {
          name: newGroupname,
  ...
  const handleDeleteGroup = async () => {
    ...
    if (confirmation) {
      try {
        const response = await axios.delete(`${apiBaseUrl}/api/delete-groups/${group_id}`);
  ...
  useEffect(() => {
    axios.get(`${apiBaseUrl}/api/groups/user/${userId}`)
    .then((response) => {
 ...
 axios.get(`${apiBaseUrl}/api/groups/user/${userId}`)
  ...
  useEffect(() => {
    //const limit = 3; 
    axios.get(`${apiBaseUrl}/api/groups-most-members/${limit}`)
      .then((response) => {
      ...
      <div className="groups-container">
        ...
        <div>
          <div className="groups-info">
            ...
            <Popup3
              title="My Groups"
              fetchFunction={() => `${apiBaseUrl}/api/groups/user/${userId}`}
              renderItem={(group) => (
          ...
          <div>
            ...
            <div className="groups-info">
              ...
              <Popup3
                title="All Groups"
                fetchFunction={() => `${apiBaseUrl}/api/groups`}
                renderItem={(group) => (
                  <span key={group.group_id}>{group.name}</span>
                )}
            ...
            <div className="groups-info">
              ...
              <Popup3
                ...
                fetchFunction={() => `${apiBaseUrl}api/groups-most-members/${limit}`}
                renderItem={(group) => (
                  <span key={group.group_id}>{group.name}</span>
                )}
