...
const User = ({ userId, username, onLogout }) => {
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${apiBaseUrl}/api/user/${userId}`, formData, {
        headers: {
...
const updateUserUsername = async (userId, newUsername) => {
    try {
      ...
      const response = await axios.post(`${apiBaseUrl}/api/username/${userId}`, { username: newUsername });
        console.log(username);
  ...
  const handleChangePassword = async () => {
    ...
    if (newPassword) {
      try {
        ...
        const response = await fetch(`${apiBaseUrl}/api/password/${userId}`, {
          method: 'POST',
          headers: {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (confirmation) {
      try {
        const response = await axios.delete(`${apiBaseUrl}/api/user/${userId}`);
  ...
  return (
    <>
        ...
        >
          ...
          <div>
            ...
            <Popup
              ...
              fetchFunction={() => `${apiBaseUrl}/api/groups/user/${userId}`} // HÖÖ Vaihda oikea polku!!
              renderItem={(group) => (
      ...
      <label>My Favorites:</label>
        ...
        fetchFunction={() => `${apiBaseUrl}/api/favorites/${userId}`}
        renderItem={(favorite) => (
